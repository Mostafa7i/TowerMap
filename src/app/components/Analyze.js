    ...
    <div className="group bg-slate-800/50 border border-slate-700/50 rounded-xl p-2 hover:border-slate-600/50 transition-all duration-200 backdrop-blur-sm">
      <div className="flex justify-between items-start gap-2 mb-3">
        <div className="overflow-hidden">
          <p className="text-xs text-slate-500 font-mono text-nowrap  uppercase tracking-widest mb-1">{label}</p>
    ...
    <div ref={analyzeRef} className="min-h-screen text-slate-100 relative" dir="rtl">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1  lg:grid-cols-4 gap-3">
          ...
          {/* ── LEFT PANEL ── */}
          <div className="lg:col-span-1 bg-slate-800/50 border border-slate-700/50 rounded-2xl p-2 backdrop-blur-sm space-y-5">
                ...
                {!result ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 backdrop-blur-sm">
                      ...
                      {/* Time to Failure / Predictive Analytics */}
                      <div className="mt-5 border-t border-slate-700/50 pt-4 cursor-default">
                        <p className="text-[9px] font-mono text-cyan-400 mb-3 uppercase tracking-widest flex items-center gap-2">
                          <Clock size={12} /> التنبؤ الزمني لانهيار الخدمة (Predictive Failure)
                        </p>
                        <div className={`p-3 rounded-xl border backdrop-blur-sm ${prob >= 75 ? "bg-red-500/10 border-red-500/30" : prob >= 40 ? "bg-orange-500/10 border-orange-500/30" : "bg-emerald-500/10 border-emerald-500/30"}`}>
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-[11px] font-mono text-slate-300 tracking-wider">EST. TIME TO CRITICAL</span>
                            <span className="text-xs font-bold font-mono" style={{ color: prob >= 75 ? "#ef4444" : prob >= 40 ? "#fb923c" : "#22c55e" }}>
                              {prob >= 75 ? "< 1 Hour (IMMINENT)" : prob >= 40 ? "~ 14 Hours" : "> 7 Days (Stable)"}
                            </span>
                          </div>
                          <div className="h-1.5 bg-slate-800/80 rounded-full overflow-hidden mt-3 shadow-inner">
                            <div className="h-full rounded-full transition-all duration-1000 relative"
                              style={{
                                width: prob >= 75 ? "95%" : prob >= 40 ? "65%" : "15%",
                                background: prob >= 75 ? "#ef4444" : prob >= 40 ? "#fb923c" : "#22c55e",
                                boxShadow: `0 0 10px ${prob >= 75 ? "#ef4444" : prob >= 40 ? "#fb923c" : "#22c55e"}80`
                              }}>
                              <div className="absolute inset-0 bg-white/20 w-full animate-pulse" />
                            </div>
                          </div>
                          <p className="text-[9px] font-mono text-slate-500 mt-3 leading-relaxed">
                            {prob >= 75 ? "تحذير: النظام يتوقع توقف كامل للشبكة إذا استمرت مؤشرات فقدان البيانات بالتدهور." : "يتم حساب التنبؤ بناءً على سلوك الأبراج المشابهة وخوارزميات Machine Learning."}
                          </p>
                        </div>
                      </div>
